              ...
              >
                <Dialog.Panel className="pointer-events-auto w-screen max-w-md">
                  <div className="flex h-full flex-col bg-white shadow-2xl overflow-hidden">
                    {/* Header Fijo */}
                    <div className="px-6 py-4 border-b border-primary-light flex items-center justify-between shrink-0 bg-white z-10">
                      <Dialog.Title className="text-xl font-serif text-primary-dark flex items-center gap-2">
                        <ShoppingBag className="w-5 h-5 text-primary" /> Mi Carrito
                      </Dialog.Title>
                      <button
                        onClick={() => setIsOpen(false)}
                        className="text-primary hover:text-primary-dark hover:rotate-90 transition-all p-1 rounded-lg cursor-pointer"
                      >
                        <X className="h-6 w-6" />
                      </button>
                    </div>

                    {/* CUERPO PRINCIPAL CON SCROLL UNIFICADO FLUIDO */}
                    <div className="flex-1 overflow-y-auto min-h-0 px-6 py-5">
                      {!showCheckoutForm ? (
                        ...
                        /* PASO 2: FORMULARIO DE FACTURACIÓN CON SCROLL COMPLETO FLUIDO */
                        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-200 pb-8">
                          <div className="flex items-center justify-between pb-2 border-b border-primary-light/60">
                            <button
                              type="button"
                          ...
                          <div className="bg-primary-light/30 border border-primary-light rounded-xl p-3 space-y-1">
                            <p className="text-[11px] font-bold text-primary-dark uppercase tracking-wider flex items-center justify-between">
                              <span>Resumen del Pedido ({cart.reduce((a, c) => a + c.qty, 0)} ítems)</span>
                              <span>${getTotal("USD").toFixed(2)} USD</span>
                            </p>
